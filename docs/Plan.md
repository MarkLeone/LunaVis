...
1. **Original Design Plan** (below) — Generated by Gemini using deep research sources in Google Notebook LM. Based on initial developer requirements and architectural goals.

2. **Implementation Plan** (appended) — Generated by Claude Code based on the original design plan and a developer interview. The same developer interview also informed the Notebook LM plan development.
